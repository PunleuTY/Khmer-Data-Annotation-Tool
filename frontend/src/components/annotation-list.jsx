...
//   }
  ...
  const runOcrAll = async () => {
    ...
    for (let i = 0; i < annotations.length; i++) {
      ...
      setProgress({ status: t("annotate.processing"), pct: Math.round(((i) / annotations.length) * 100) })
    ...
    <Card>
      <CardHeader className="pb-3 flex flex-row items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {busy && (
            <>
              <div className="w-40"><Progress value={progress?.pct || 0} /></div>
              <span className="text-xs text-gray-600">{progress?.pct || 0}%</span>
              <Button variant="outline" size="sm" onClick={cancelBatch}>
                {t("annotate.cancel")}
              </Button>
            </>
          )}
          <Button variant="outline" size="sm" onClick={runOcrAll} disabled={!annotations.length || busy}>
          ...
          <div key={a.id} className="p-3 rounded-md border">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-700">
                {a.type === "box" ? t("annotate.box") : t("annotate.polygon")} #{idx + 1}
              </div>
              <div className="flex items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={() => runOcr(a.id)} disabled={busy}>
                  <Play className="w-4 h-4 mr-2" />
                  {t("annotate.runOcr")}
                </Button>
                <Button variant="destructive" size="icon" onClick={() => onDelete(a.id)} aria-label={t("annotate.delete")}>
            ...
            <div className="mt-3 grid md:grid-cols-3 gap-3">
              <div>
                <label className="text-xs text-gray-600 block mb-1">{t("annotate.label")}</label>
                <Input
                  value={a.label || ""}
              ...
              <div className="md:col-span-2">
                <label className="text-xs text-gray-600 block mb-1">{t("annotate.extracted")}</label>
                <Textarea value={a.text || ""} onChange={(e) => onUpdate(a.id, { text: e.target.value })} rows={3} />
              </div>
            </div>

            <div className="mt-3 grid md:grid-cols-3 gap-3">
              <div className="md:col-span-3">
                <label className="text-xs text-gray-600 block mb-1">{t("annotate.groundTruth")}</label>
                <Textarea
                  value={a.gt || ""}
